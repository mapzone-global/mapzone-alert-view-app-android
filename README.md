...
When a segment fetch reaches the Vietmap server, the outcome arrives through the
**result callback** `errorCode`:

| Code | Constant | Meaning / when it happens |
|---|---|---|
| `0` | Success | OK. |
| `1000` | InvalidRequest | Malformed request. |
| `1001` | InvalidParameter | A route point falls outside Vietnam (`lng∈[102,110]`, `lat∈[8,24]`). |
| `1002` | MissingParameter | No route points were sent. |
| `1003` | InvalidFormat | Wrong data format. |
| `1004` | UnsupportedValue | Unsupported value. |
| `2000` | RequestExpired | Auth timestamp drift. |
| `2001` | InvalidTimestamp | Negative / unparseable timestamp. |
| `2003` | Unauthorized | Reserved. |
| `2004` | Forbidden | Reserved. |
| `3003` | VehicleTypeNotSupported | `vehicleType` outside 1..9. |
| `3004` | NoRouteData | The route could not be map-matched to any road link. |
| `5000` | InternalServerError | Unclassified server error. |
| `5003` | EncryptionError | Server-side encryption error (reserved). |
| `5999` | UnknownError | Unknown error. |
